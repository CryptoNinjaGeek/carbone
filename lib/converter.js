...
var isLibreOfficeDetectionRun = false;
...
var converter = {
  ...
  convertFile : function (inputFile, outputType, formatOptions, outputFile, callback) {
    ...
    if (isLibreOfficeDetectionRun === false) {
      detectLibreOffice();
    }
...
function detectLibreOffice (additionalPaths) {
  isLibreOfficeDetectionRun = true;
  ...
  if (process.platform === 'darwin') {
    ...
    _pathsToCheck = _pathsToCheck.concat(_listProgramDirectories('/usr/lib', _linuxDirnamePattern));
    console.log(_pathsToCheck)
  }
